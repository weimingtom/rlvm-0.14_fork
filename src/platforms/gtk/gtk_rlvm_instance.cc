...
printf("<<<<<< on_selection_changed 2 f, %s\n", path.c_str());		
    gtk_dialog_set_response_sensitive(
        GTK_DIALOG(chooser), GTK_RESPONSE_ACCEPT, fs::exists(path));
    g_free(name);
  }
printf("<<<<<< on_selection_changed end\n");
}
...
//see https://github.com/xyzz/rlvm/blob/android/src/platforms/android/rlvm.cpp

namespace fs = boost::filesystem;

GtkRLVMInstance::GtkRLVMInstance(int* argc, char** argv[]) {}
GtkRLVMInstance::~GtkRLVMInstance() {}
boost::filesystem::path GtkRLVMInstance::SelectGameDirectory() {
  boost::filesystem::path out_path = ".";//"/home/wmt/planetarian_jp";
  DoNativeWork();
  return out_path;
}
