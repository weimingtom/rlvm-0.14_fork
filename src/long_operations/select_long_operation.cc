      ...
      switch (condition.effect) {
        ...
        case SelectElement::OPTION_HIDE: {
          ...
          if (condition.condition != "") {
            ...
            std::unique_ptr<ExpressionPiece> condition(libreallive::GetExpression(location));
...
void ButtonSelectLongOperation::Render(std::ostream* tree) {
  for (size_t i = 0; i < buttons_.size(); i++) {
    ...
    if (name_surface_) {
Rect dstRect;
calcScreenRect(bounding_rect, dstRect);
      name_surface_->RenderToScreen(name_surface_->GetPattern(frame).rect,
                                    dstRect, 255, __FILE__, __LINE__);
    }
...
Rect dstRect;
calcScreenRect(text_bounding_rect, dstRect);
  text_surface->RenderToScreen(text_surface->GetRect(), dstRect, 255, __FILE__, __LINE__);
}
