...
void RLOperation::DispatchFunction(RLMachine& machine,
  ...
  const std::vector<std::unique_ptr<libreallive::ExpressionPiece>>& parameter_pieces = 
...
IntReference_T::type IntReference_T::getData(
    ...
    const std::vector<std::unique_ptr<libreallive::ExpressionPiece>>& p,
    unsigned int& position) {
  return static_cast<const libreallive::MemoryReference&>(*p[position++]).GetIntegerReferenceIterator(machine);
}
...
StrReference_T::type StrReference_T::getData(
    ...
    const std::vector<std::unique_ptr<libreallive::ExpressionPiece>>& p,
    unsigned int& position) {
  return static_cast<const libreallive::MemoryReference&>(*p[position++]).GetStringReferenceIterator(machine);
}
