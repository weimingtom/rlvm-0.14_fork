...
Rect textOrigin_rect(textOrigin, surface_size);	  
Rect dstRect;
calcScreenRect(textOrigin_rect, dstRect);

      text_surface->RenderToScreen(
          Rect(Point(0, 0), surface_size), dstRect, 255, __FILE__, __LINE__);
  ...
  for (int i = 0; i < kNumFaceSlots; ++i) {
    if (face_slot_[i] && face_slot_[i]->face_surface &&
      ...
      Rect dest(GetWindowRect().x() + face_slot_[i]->x,
                ...
                surface->GetSize());
      surface->RenderToScreen(surface->GetRect(), dest, 255, __FILE__, __LINE__);
