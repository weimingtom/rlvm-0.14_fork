...
printf("<<<<====222, %s is_dir %d\n", dir->path().filename().string().c_str(), 1);
      to_lower(lowername);
...
printf("<<<<====222, %s is_dir %d\n", dir->path().filename().string().c_str(), 0);	
	}
  }
printf("<<<<====closedir, %s\n", gamepath.c_str());
}
...
printf("<<<<====opendir AddDirectoryToCache, %s\n", directory.c_str());
  fs::directory_iterator dir_end;
...
#else
        filesystem_cache_.insert(make_pair(stem, make_pair(extension, dir->path())));
#endif
      }
    }
printf("<<<<====AddDirectoryToCache closedir, %s\n", directory.c_str());
  }
}
