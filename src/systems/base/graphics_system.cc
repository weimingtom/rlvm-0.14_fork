...
void GraphicsSystem::DrawFrame(std::ostream* tree) {
  switch (background_type_) {
    case BACKGROUND_DC0: {
Rect dstRect;
calcScreenRect(screen_rect(), dstRect);
      // Display DC0
      GetDC(0)->RenderToScreen(screen_rect(), dstRect, 255, __FILE__, __LINE__);
      if (tree) {
    ...
    case BACKGROUND_HIK: {
      if (hik_renderer_) {
        ...
        GetHaikei()->RenderToScreen(screen_rect(), screen_rect(), 255, __FILE__, __LINE__);
        if (tree) {
