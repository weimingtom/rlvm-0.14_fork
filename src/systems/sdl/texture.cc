...
#if DEBUG_SURFACE 
#include <SDL2/SDL_image.h> //for IMG_SavePNG
#endif
...
#else
//use GL_TEXTURE_WRAP_T to stop OpenGL adding black border line (or curves) 
//around the textures with alpha channel if scaled
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); //GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE); //GL_REPEAT);
...
//=======================
//FIXME:don't modify the pixels, use GL_TEXTURE_WRAP_T instead
//
//        memset(cur_dst_ptr, 255, subrow_size);
//#if 0
//for (int i = 0; i < subrow_size / 4; ++i)
//{
////i % 4 == 0==blue,1==green,2==red,3==alpha
//if (1)//cur_dst_ptr[i * 4 + 3] == 0)
//{
//cur_dst_ptr[i * 4 + 0] = 255;
//cur_dst_ptr[i * 4 + 1] = 0;
//cur_dst_ptr[i * 4 + 2] = 0;
//cur_dst_ptr[i * 4 + 3] = 255;
//}
//
//}
//#endif
...
char str[256] = {0};
sprintf(str, "%d.png", texture_index);
texture_index++;
#if DEBUG_SURFACE
...
//=======================
//FIXME:don't modify the pixels, use GL_TEXTURE_WRAP_T instead		
//but, don't modify here, just test code
//
//        memset(cur_dst_ptr, 255, subrow_size);
//=======================
        cur_dst_ptr += subrow_size;
...
//=======================
//test code dump png 1/2
//actually cover picture not run here, run upper code
if (texture_index == 15)
{
  printf("--->15\n");
}
...
void Texture::RenderToScreen(const Rect& src, const Rect& dst, int opacity, const char *filename, int lineno) {
//printf("xx<<<<<<<<<<<RenderToScreen, %d, %d, %d, %d, %s, %d\n", dst.x(), dst.y(), dst.x2(), dst.y2(), filename, lineno);
#if !MY_USE_MINLIB
  int x1 = src.x(), y1 = src.y(), x2 = src.x2(), y2 = src.y2();
//see systems/base/mouse_cursor.cc, MouseCursor::RenderHotspotAt
//render font and bottom dialog button
...
void Texture::RenderToScreenAsColorMask(const Rect& src,
                                        ...
                                        int filter) {
printf("<<<<<<<<<<<RenderToScreenAsColorMask\n");
#if !MY_USE_MINLIB                                       
  if (filter == 0) {
#if !MY_USE_GLES2
...
void Texture::render_to_screen_as_colour_mask_subtractive_glsl(
    ...
    const Rect& dst,
    const RGBAColour& rgba) {
//printf("<<<<<<<<<<<render_to_screen_as_colour_mask_subtractive_glsl\n");
#if !MY_USE_MINLIB      
  int x1 = src.x(), y1 = src.y(), x2 = src.x2(), y2 = src.y2();
  int fdx1 = dst.x(), fdy1 = dst.y(), fdx2 = dst.x2(), fdy2 = dst.y2();
calcScreenRect(fdx1, fdy1, fdx2, fdy2);  
  if (!filterCoords(x1, y1, x2, y2, fdx1, fdy1, fdx2, fdy2))
    return;
...
void Texture::render_to_screen_as_colour_mask_subtractive_fallback(
    ...
    const Rect& dst,
    const RGBAColour& rgba) {
//printf("<<<<<<<<<<<render_to_screen_as_colour_mask_subtractive_fallback\n");
#if !MY_USE_MINLIB      
  int x1 = src.x(), y1 = src.y(), x2 = src.x2(), y2 = src.y2();
  int fdx1 = dst.x(), fdy1 = dst.y(), fdx2 = dst.x2(), fdy2 = dst.y2();
calcScreenRect(fdx1, fdy1, fdx2, fdy2);
...
void Texture::render_to_screen_as_colour_mask_additive(const Rect& src,
                                                       const Rect& dst,
                                                       const RGBAColour& rgba) {
//printf("<<<<<<<<<<<render_to_screen_as_colour_mask_additive\n");
#if !MY_USE_MINLIB  
  int x1 = src.x(), y1 = src.y(), x2 = src.x2(), y2 = src.y2();
  int fdx1 = dst.x(), fdy1 = dst.y(), fdx2 = dst.x2(), fdy2 = dst.y2();
calcScreenRect(fdx1, fdy1, fdx2, fdy2);   
...
void Texture::RenderToScreen2(const Rect& src,
                             const Rect& dst,
                             const int opacity[4], const char *filename, int lineno) {
//printf("<<<<<<<<<<<RenderToScreen2\n");
...
void Texture::RenderToScreenAsObject(const GraphicsObject& go,
                                     ...
                                     const Rect& srcRect,
                                     const Rect& dstRect,
                                     int alpha, const char *filename, int lineno) {
//printf("<<<<<<<<<<<RenderToScreenAsObject %s, %d, alpha == %d, dstRect(%d,%d,%d,%d)\n", filename, lineno, alpha, dstRect.x(), dstRect.y(), dstRect.width(), dstRect.height());

//======================
//test code dump png 2/2
if (alpha == 255 && dstRect.x() == 700 && dstRect.y() == 412)
{
  cur_index = 0;
}
else if (cur_index >= 0)
...
if (cur_index > 5)
{
//  if (cur_index == 6 || cur_index == 7 || cur_index == 8 || cur_index == 9 || cur_index == 10 || cur_index == 11 || cur_index == 12) 
//  {
//    return; //hidden
//  }

//if (cur_index == 6) return; //hidden

} 


//game begin white screen run here 
//return;
//======================
