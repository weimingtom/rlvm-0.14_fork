...
#if USE_SDL2
//FIXME:if use_src_alpha->0 ??? can solve : why font not shown
#endif
...
void SDLSurface::RenderToScreen(const Rect& src,
                                const Rect& dst,
                                int alpha, const char *filename, int lineno) const {
  uploadTextureIfNeeded();

  for (std::vector<TextureRecord>::iterator it = textures_.begin();
       it != textures_.end();
       ++it) {
    it->texture->RenderToScreen(src, dst, alpha, filename, lineno);
  }
}
...
void SDLSurface::RenderToScreen2(const Rect& src,
                                const Rect& dst,
                                const int opacity[4], const char *filename, int lineno) const {
  uploadTextureIfNeeded();

  for (std::vector<TextureRecord>::iterator it = textures_.begin();
       it != textures_.end();
       ++it) {
    it->texture->RenderToScreen2(src, dst, opacity, filename, lineno);
  }
}
...
void SDLSurface::RenderToScreenAsObject(const GraphicsObject& rp,
  ...
  for (std::vector<TextureRecord>::iterator it = textures_.begin();
       it != textures_.end();
       ++it) {
    it->texture->RenderToScreenAsObject(rp, *this, src, dst, alpha, filename, lineno);
  }
}
