...
#if MY_USE_GLES2
//not need actually
    glViewport(0, 0, (GLint) screen_size().width(), (GLint) screen_size().height());
#endif
...
#if !MY_USE_GLES2 // broken, for now
...
#if !MY_USE_MINLIB && !MY_USE_GLES2 // TODO(xyz): this makes clicks (in main menu) unreliable, wtf?
...
#if USE_SDL2
#if !defined(__APPLE__) && !defined(_WIN32) and !MY_USE_GLES2
...
#endif
#else
#if defined(__linux__) and !MY_USE_GLES2
  // We only set the icon on linux because OSX will use the icns file
  // automatically and this doesn't look too awesome.
  SDL_Surface* icon = IMG_Load("/usr/share/icons/hicolor/48x48/apps/rlvm.png");
  if (icon) {
...
#if defined(MIYOO)
//SetScreenMode(1); //enter full screen mode
#endif
}
...
//#endif
Size fulll_screen_size;
getScreenRect(fulll_screen_size);
  window_ = SDL_CreateWindow("rlvm",
                             SDL_WINDOWPOS_UNDEFINED,
                             SDL_WINDOWPOS_UNDEFINED,
#if MY_USE_GLES2 //defined(MIYOO)
//calcScreenRect
                             fulll_screen_size.width(),
                             fulll_screen_size.height(),
#else
                             screen_size().width(),
                             screen_size().height(),
#endif
...
#if !MY_USE_GLES2
#else
  SDL_SetWindowTitle(window_, "rlvm");
...
//  jint w = screen_size().width();
//  jint h = screen_size().height();
...
//Size fulll_screen_size;
//getScreenRect(fulll_screen_size);
  screen_tex_width_ = SafeSize(fulll_screen_size.width());
  screen_tex_height_ = SafeSize(fulll_screen_size.height());
#else
